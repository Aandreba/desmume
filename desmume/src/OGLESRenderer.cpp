...
size_t OpenGLRenderer::DrawPolygonsForIndexRange(
    const POLYLIST *polyList, const INDEXLIST *indexList, size_t firstIndex,
    size_t lastIndex, size_t &indexOffset, POLYGON_ATTR &lastPolyAttr) {
...
Render3DError OpenGLRenderer_1_2::CreateToonTable() {
  ...
  glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB, 32, 0, GL_RGBA,
               GL_UNSIGNED_SHORT_1_5_5_5_REV, tempToonTable);
...
Render3DError OpenGLRenderer_1_2::ZeroDstAlphaPass(const POLYLIST *polyList,
  ...
  glUniform1i(OGLRef.uniformTexDrawOpaque[this->_geometryProgramFlags.value],
              GL_FALSE);
  ...
  this->DrawPolygonsForIndexRange<OGLPolyDrawMode_ZeroAlphaPass>(
      polyList, indexList, this->_clippedPolyOpaqueCount,
  ...
  glUseProgram(OGLRef.programGeometryID[this->_geometryProgramFlags.value]);
  glDrawBuffers(
      3, GeometryDrawBuffersList[this->_geometryProgramFlags.DrawBuffersMode]);
...
Render3DError OpenGLRenderer_1_2::BeginRender(const GFX3D &engine) {
  ...
  for (size_t i = 0, vertIndexCount = 0; i < this->_clippedPolyCount; i++) {
    ...
    this->_renderNeedsDepthEqualsTest =
        this->_renderNeedsDepthEqualsTest ||
  ...
  if (this->isShaderSupported) {
    ...
    this->_geometryProgramFlags.NeedsDepthEqualsTest =
        (this->_renderNeedsDepthEqualsTest) ? 1 : 0;
    ...
    glUniform1f(
        OGLRef.uniformStateAlphaTestRef[this->_geometryProgramFlags.value],
...
Render3DError OpenGLRenderer_1_2::RenderGeometry(const GFX3D_State &renderState,
  ...
  if (this->_clippedPolyCount > 0) {
    ...
    if (this->_clippedPolyOpaqueCount > 0) {
      ...
      this->DrawPolygonsForIndexRange<OGLPolyDrawMode_DrawOpaquePolys>(
          polyList, indexList, 0, this->_clippedPolyOpaqueCount - 1,
    ...
    if (this->_clippedPolyOpaqueCount < this->_clippedPolyCount) {
      if (this->_needsZeroDstAlphaPass &&
        ...
        this->ZeroDstAlphaPass(polyList, indexList,
                               renderState.enableAlphaBlending, indexOffset,
                               lastPolyAttr);
...
Render3DError OpenGLRenderer_1_2::RenderEdgeMarking(const u16 *colorTable,
                                                    const bool useAntialias) {
  ...
  const GLfloat oglColor[4 * 8] = {
      divide5bitBy31_LUT[(colorTable[0]) & 0x001F],
...
Render3DError OpenGLRenderer_1_2::RenderFog(const u8 *densityTable,
                                            const u32 color, const u16 offset,
                                            const u8 shift,
  ...
  if (it == this->_fogProgramMap.end()) {
    Render3DError error = this->CreateFogProgram(
        fogProgramKey, FogVtxShader_100, FogFragShader_100);
    if (error != OGLERROR_NOERR) {
      return error;
    }
  }
  ...
  for (size_t i = 0; i < 32; i++) {
    oglDensityTable[i] =
        (densityTable[i] == 127) ? 1.0f : (GLfloat)densityTable[i] / 128.0f;
  }
  ...
  glUniform1i(OGLRef.uniformStateEnableFogAlphaOnly,
              (alphaOnly) ? GL_TRUE : GL_FALSE);
  glUniform4f(OGLRef.uniformStateFogColor, oglColor[0], oglColor[1],
              oglColor[2], oglColor[3]);
...
Render3DError OpenGLRenderer_1_2::UpdateToonTable(const u16 *toonTableBuffer) {
  if (this->isShaderSupported) {
    ...
    glTexSubImage1D(GL_TEXTURE_1D, 0, 0, 32, GL_RGBA,
                    GL_UNSIGNED_SHORT_1_5_5_5_REV, toonTableBuffer);
  }
  ...
  if (this->isMultisampledFBOSupported) {
    ...
    if (OGLRef.selectedRenderingFBO == OGLRef.fboMSIntermediateRenderID) {
      ...
      if (this->isShaderSupported) {
        ...
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, OGLRef.selectedRenderingFBO);
        glDrawBuffers(3, GeometryDrawBuffersList[this->_geometryProgramFlags
                                                     .DrawBuffersMode]);
      } else {
        // Blit the color and depth buffers.
        glBlitFramebufferEXT(
