...
static unsigned host_get_language(void) {
  static const u8 langconv[] = {// libretro to NDS
                                NDS_FW_LANG_ENG, NDS_FW_LANG_JAP,
                                NDS_FW_LANG_FRE, NDS_FW_LANG_SPA,
                                NDS_FW_LANG_GER, NDS_FW_LANG_ITA};
...
static void DrawPointerLineSmall_32(uint32_t *aOut, uint32_t aPitchInPix,
                                    int factor) {
...
static void DrawPointer(uint16_t *aOut, uint32_t aPitchInPix) {
  ...
  if (colorMode != RETRO_PIXEL_FORMAT_XRGB8888) {
    if (TouchX > (5 * scale))
      DrawPointerLine(&aOut[TouchY * aPitchInPix + TouchX - (5 * scale)], 1);
    if (TouchX < (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH - (5 * scale)))
      DrawPointerLine(&aOut[TouchY * aPitchInPix + TouchX + 1], 1);
    if (TouchY > (5 * scale))
      DrawPointerLine(&aOut[(TouchY - (5 * scale)) * aPitchInPix + TouchX],
                      aPitchInPix);
    if (TouchY < (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - (5 * scale)))
      DrawPointerLine(&aOut[(TouchY + 1) * aPitchInPix + TouchX], aPitchInPix);
  } else {
    uint32_t *aOut_32 = (uint32_t *)aOut;
    if (TouchX > (5 * scale))
      DrawPointerLine_32(&aOut_32[TouchY * aPitchInPix + TouchX - (5 * scale)],
                         1);
    if (TouchX < (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH - (5 * scale)))
      DrawPointerLine_32(&aOut_32[TouchY * aPitchInPix + TouchX + 1], 1);
    if (TouchY > (5 * scale))
      DrawPointerLine_32(
          &aOut_32[(TouchY - (5 * scale)) * aPitchInPix + TouchX], aPitchInPix);
    if (TouchY < (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - (5 * scale)))
      DrawPointerLine_32(&aOut_32[(TouchY + 1) * aPitchInPix + TouchX],
                         aPitchInPix);
  }
}

static void DrawPointerHybrid(uint16_t *aOut, uint32_t aPitchInPix,
                              bool large) {
  ...
  if (!large) {
    aOut += aPitchInPix * hybrid_layout_scale * bpp / 2 *
            (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT / hybrid_layout_ratio +
             gap_size() * scale);
    width = GPU_LR_FRAMEBUFFER_NATIVE_WIDTH * hybrid_layout_scale /
            hybrid_layout_ratio;
    height = GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT * hybrid_layout_scale /
             hybrid_layout_ratio;

    DrawX = Saturate(0, (width - 1),
                     TouchX * hybrid_layout_scale / hybrid_layout_ratio);
    DrawY = Saturate(0, (height - 1),
                     TouchY * hybrid_layout_scale / hybrid_layout_ratio);
  } else {
  ...
  if (colorMode != RETRO_PIXEL_FORMAT_XRGB8888) {
    if (DrawX > (factor * scale))
      DrawPointerLineSmall(
          &aOut[DrawY * aPitchInPix + DrawX - (factor * scale)], 1, factor);
    if (DrawX < (width - (factor * scale)))
      DrawPointerLineSmall(&aOut[DrawY * aPitchInPix + DrawX + 1], 1, factor);
    if (DrawY > (factor * scale))
      DrawPointerLineSmall(
          &aOut[(DrawY - (factor * scale)) * aPitchInPix + DrawX], aPitchInPix,
          factor);
    if (DrawY < (height - (factor * scale)))
      DrawPointerLineSmall(&aOut[(DrawY + 1) * aPitchInPix + DrawX],
                           aPitchInPix, factor);
  } else {
    uint32_t *aOut_32 = (uint32_t *)aOut;
    if (DrawX > (factor * scale))
      DrawPointerLineSmall_32(
          &aOut_32[DrawY * aPitchInPix + DrawX - (factor * scale)], 1, factor);
    if (DrawX < (width - (factor * scale)))
      DrawPointerLineSmall_32(&aOut_32[DrawY * aPitchInPix + DrawX + 1], 1,
                              factor);
    if (DrawY > (factor * scale))
      DrawPointerLineSmall_32(
          &aOut_32[(DrawY - (factor * scale)) * aPitchInPix + DrawX],
          aPitchInPix, factor);
    if (DrawY < (height - (factor * scale)))
      DrawPointerLineSmall_32(&aOut_32[(DrawY + 1) * aPitchInPix + DrawX],
                              aPitchInPix, factor);
  }
}
...
#define CONVERT_COLOR(color)                                                   \
  (((color & 0x001f) << 11) | ((color & 0x03e0) << 1) |                        \
   ((color & 0x0200) >> 4) | ((color & 0x7c00) >> 10))
void conv_0rgb1555_rb_swapped_rgb565(void *__restrict output_,
                                     const void *__restrict input_, int width,
                                     int height, int out_stride,
                                     int in_stride) {
  int h, max_width;
...
static void SwapScreen(uint16_t *dst, const uint16_t *src, uint32_t pitch) {
  conv_0rgb1555_rb_swapped_rgb565(dst, src, GPU_LR_FRAMEBUFFER_NATIVE_WIDTH,
                                  GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT, pitch,
...
static void SwapScreen_32(uint32_t *dst, const uint32_t *src, uint32_t pitch) {
  ...
  for (i = 0; i < GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT; i++) {
    memcpy(dst + i * pitch, src + i * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH,
           GPU_LR_FRAMEBUFFER_NATIVE_WIDTH * 4);
  }
}

static void SwapScreenLarge_32(uint32_t *dst, const uint32_t *src,
                               uint32_t pitch) {
  unsigned y, x, z;
  ...
  for (y = 0; y < GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT; y++) {
    ...
    for (x = 0; x < GPU_LR_FRAMEBUFFER_NATIVE_WIDTH; x++) {
      for (z = 0; z < hybrid_layout_scale; z++)
        out[x * hybrid_layout_scale + z] =
            src[y * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH + x];
    }

    for (z = 1; z < hybrid_layout_scale; z++)
      memcpy(dst + ((y * hybrid_layout_scale) + z) * pitch, out,
             GPU_LR_FRAMEBUFFER_NATIVE_WIDTH * hybrid_layout_scale * 4);
  }
}

static void SwapScreenSmall_32(uint32_t *dst, const uint32_t *src,
                               uint32_t pitch, bool first, bool draw) {
  unsigned x, y;
  ...
  if (!first) {
    int screenheight = GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT * hybrid_layout_scale /
                       hybrid_layout_ratio;
  ...
  if (hybrid_layout_scale != hybrid_layout_ratio) {
    ...
    for (y = 0; y < GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT / hybrid_layout_ratio;
         y++) {
      for (x = 0; x < GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / hybrid_layout_ratio;
           x++) {
        *dst++ = src[hybrid_layout_ratio *
                     (y * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH + x)];
      }
      dst += GPU_LR_FRAMEBUFFER_NATIVE_WIDTH;
    }
  } else {
    for (y = 0; y < GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT; y++) {
      memcpy(dst, src + y * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH,
...
static void SwapScreenLarge(uint16_t *dst, const uint16_t *src,
                            uint32_t pitch) {
  unsigned y, x, z;
  ...
  for (y = 0; y < GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT; y++) {
    ...
    for (x = 0; x < GPU_LR_FRAMEBUFFER_NATIVE_WIDTH; x++) {
      for (z = 0; z < hybrid_layout_scale; z++)
        out[x * hybrid_layout_scale + z] =
            CONVERT_COLOR(src[y * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH + x]);
    }

    for (z = 1; z < hybrid_layout_scale; z++)
      memcpy(dst + ((y * hybrid_layout_scale) + z) * pitch, out,
             GPU_LR_FRAMEBUFFER_NATIVE_WIDTH * hybrid_layout_scale * 2);
  }
}

static void SwapScreenSmall(uint16_t *dst, const uint16_t *src, uint32_t pitch,
                            bool first, bool draw) {
  unsigned x, y;
  ...
  if (hybrid_layout_scale != hybrid_layout_ratio) {
    ...
    for (y = 0; y < GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT / hybrid_layout_ratio;
      ...
      for (x = 0; x < GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / hybrid_layout_ratio;
           x++) {
        *dst++ = CONVERT_COLOR(src[hybrid_layout_ratio *
                                   (y * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH + x)]);
      }
      dst += GPU_LR_FRAMEBUFFER_NATIVE_WIDTH;
    }
  } else {
    conv_0rgb1555_rb_swapped_rgb565(
        dst, src,
        (pitch - GPU_LR_FRAMEBUFFER_NATIVE_WIDTH * hybrid_layout_ratio),
        GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT, pitch,
        GPU_LR_FRAMEBUFFER_NATIVE_WIDTH);
...
static void update_layout_params(unsigned id, LayoutData *layout) {
  ...
  case LAYOUT_HYBRID_BOTTOM_ONLY:
    ...
    layout->width =
        hybrid_layout_scale * (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH + awidth);
    ...
    layout->offset1 = 0;
    {
      ...
      out += bytewidth *
             hybrid_layout_scale; // Move pointer to right by large screen width
      int pitch = layout->pitch * bpp; // byte size of a line
      int halfscreen =
          layout->height / 2; // y offset: midpoint of the screen height
      halfscreen -= (gap_size() * scale * hybrid_layout_scale) /
                    2; // move upward by half the gap height
      halfscreen -=
          GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT * hybrid_layout_scale /
          hybrid_layout_ratio;   // move y offset framebuffer height upward
...
static void check_variables(bool first_boot) {
  ...
  if (first_boot) {
    ...
    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
      CommonSettings.jit_max_block_size =
          var.value ? strtol(var.value, 0, 10) : 100;
    else
    ...
    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
      ...
      if (!strcmp(var.value, "enabled")) {
        if (environ_cb(RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY,
                       &system_directory) &&
            system_directory) {
          std::string bios7_loc = std::string(system_directory) +
                                  DIRECTORY_DELIMITER_CHAR + "bios7.bin";
          std::string bios9_loc = std::string(system_directory) +
                                  DIRECTORY_DELIMITER_CHAR + "bios9.bin";
          std::string firmware_loc = std::string(system_directory) +
                                     DIRECTORY_DELIMITER_CHAR + "firmware.bin";
    ...
    if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
      if (!strcmp(var.value, "enabled")) {
        if (CommonSettings.UseExtBIOS && !CommonSettings.use_jit) {
          ...
          log_cb(RETRO_LOG_WARN, "Cannot boot into BIOS. Must enable external "
                                 "bios and interpreter mode.\n");
        }
      }
    }
    ...
    if (opengl_mode && environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) &&
        var.value) {
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
    ...
    if (!first_boot &&
        GPU->GetCustomFramebufferWidth() != GPU_LR_FRAMEBUFFER_NATIVE_WIDTH)
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
    static const struct {
      const char *name;
      int id;
    } languages[] = {{"Auto", -1},  {"Japanese", 0}, {"English", 1},
                     {"French", 2}, {"German", 3},   {"Italian", 4},
                     {"Spanish", 5}};

    for (int i = 0; i < 7; i++) {
      if (!strcmp(languages[i].name, var.value)) {
        firmwareLanguage = languages[i].id;
        if (firmwareLanguage == -1)
          firmwareLanguage = host_get_language();
        break;
      }
    }
  } else
  ...
  if (need_framebuffer_reset) {
    GPU->SetCustomFramebufferSize(GPU_LR_FRAMEBUFFER_NATIVE_WIDTH,
...
#ifdef HAVE_OPENGL
                                &gpu3Dgl,   &gpu3DglOld,     &gpu3Dgl_3_2,
#endif
                                NULL};
...
SoundInterface_struct *SNDCoreList[] = {&SNDRetro};
...
static bool initialize_gl() {
  ...
  glBindFramebuffer =
      (glBindFramebufferProc *)hw_render.get_proc_address("glBindFramebuffer");
  glGenFramebuffers =
      (glGenFramebuffersProc *)hw_render.get_proc_address("glGenFramebuffers");
  glDeleteFramebuffers = (glDeleteFramebuffersProc *)hw_render.get_proc_address(
      "glDeleteFramebuffers");
  glFramebufferTexture2D =
      (glFramebufferTexture2DProc *)hw_render.get_proc_address(
          "glFramebufferTexture2D");
  glBlitFramebuffer =
      (glBlitFramebufferProc *)hw_render.get_proc_address("glBlitFramebuffer");
  glMapBufferRange =
      (glMapBufferRangeProc *)hw_render.get_proc_address("glMapBufferRange");

  if (!glBindFramebuffer || !glGenFramebuffers || !glDeleteFramebuffers ||
      !glFramebufferTexture2D || !glBlitFramebuffer) {
...
void retro_reset(void) { NDS_Reset(); }
...
void retro_run(void) {
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE_UPDATE, &updated) && updated) {
    ...
    /* Reinitialize the video if the layout is significantly different than
       previously set. This makes image uploads more efficient */
    if (current_max_width != 0 &&
      ...
      log_cb(RETRO_LOG_INFO,
             "Screen size changed significantly. Reinitializing.\n");
      current_max_width = new_av_info.geometry.max_width;
      current_max_height = new_av_info.geometry.max_height;
  ...
  l_analog_x_ret =
      input_cb(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_LEFT,
               RETRO_DEVICE_ID_ANALOG_X);
  l_analog_y_ret =
      input_cb(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_LEFT,
               RETRO_DEVICE_ID_ANALOG_Y);
  r_analog_x_ret =
      input_cb(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_RIGHT,
               RETRO_DEVICE_ID_ANALOG_X);
  r_analog_y_ret =
      input_cb(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_RIGHT,
               RETRO_DEVICE_ID_ANALOG_Y);

  if (pointer_device_l != 0 ||
      pointer_device_r != 0) // 1=emulated pointer, 2=absolute pointer,
                             // 3=absolute pointer constantly pressed
  {
    int16_t analogX_l = 0;
    int16_t analogY_l = 0;
    int16_t analogX_r = 0;
    ...
    if ((pointer_device_l == 1) || (pointer_device_r == 1)) {
      ...
      float final_acceleration =
          ...
          (1.0 + (float)analog_stick_acceleration_modifier / 100.0);
      ...
      if (radius > (float)analog_stick_deadzone * max / 100) {
        // Re-scale analog stick range to negate deadzone (makes slow movements
        // possible)
        radius =
            (radius - (float)analog_stick_deadzone * max / 100) *
            ((float)max / (max - (float)analog_stick_deadzone * max / 100));

        // Convert back to cartesian coordinates
        analogXpointer = (int32_t)round(radius * cos(angle));
        analogYpointer = (int32_t)round(radius * sin(angle));

        TouchX = Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH - 1),
                          TouchX + analogXpointer);
        TouchY = Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - 1),
                          TouchY + analogYpointer);
      }
    }

    // absolute pointer -- doesn't run if emulated pointer > deadzone
    if (((pointer_device_l == 2) || (pointer_device_l == 3) ||
         (pointer_device_r == 2) || (pointer_device_r == 3)) &&
        !(analogXpointer || analogYpointer)) {
      if (((pointer_device_l == 2) || (pointer_device_l == 3)) &&
          ((pointer_device_r == 2) ||
           (pointer_device_r == 3))) // both sticks set to absolute or pressed
      {
        ...
        if (pointer_device_l == 3) // left analog is always pressed
        {
          ...
          double radius =
              sqrt(analogXpress * analogXpress + analogYpress * analogYpress);
          ...
          if (radius > (float)analog_stick_deadzone * 0x8000 / 100) {
            ...
            analogX = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / 2 *
                      analogXpress / (float)0x8000;
            analogY = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT / 2 *
                      analogYpress / (float)0x8000;

          } else if (pointer_device_r == 2) // use the other stick as absolute
          {
            analogX = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / 2 *
                      r_analog_x_ret / (float)0x8000;
            analogY = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / 2 *
                      r_analog_y_ret / (float)0x8000;
            rotate_input(analogX, analogY, input_rotation);
          }
        }
        ...
        else if (pointer_device_r == 3) // right analog is always pressed
        {
          ...
          if (radius > (float)analog_stick_deadzone * 0x8000 / 100) {
            ...
            analogX = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / 2 *
                      l_analog_x_ret / (float)0x8000;
            analogY = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / 2 *
                      l_analog_y_ret / (float)0x8000;
            rotate_input(analogX, analogY, input_rotation);
          }
        ...
        TouchX =
            Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH - 1),
                     analogX + ((GPU_LR_FRAMEBUFFER_NATIVE_WIDTH - 1) / 2));
        TouchY =
            Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - 1),
                     analogY + ((GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - 1) / 2));
      }
        ...
        if (pointer_device_l == 2) {
          ...
          rotate_input(analogX, analogY, input_rotation);
          analogX = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH / 2 * analogX /
                    (float)0x8000;
          analogY = sqrt(2) * GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT / 2 * analogY /
                    (float)0x8000;
        ...
        if (pointer_device_l == 3) {
          ...
          if (radius > (float)analog_stick_deadzone * (float)0x8000 / 100) {
            ...
            TouchY = Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - 1),
                              analogY +
                                  ((GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - 1) / 2));
          }
        }
      }
    ...
    FramesWithPointer =
        (analogX || analogY) ? FramesWithPointerBase : FramesWithPointer;
  }
  ...
  if (mouse_enable) {
    ...
    if (!touchEnabled) {
      ...
      have_touch = have_touch || input_cb(0, RETRO_DEVICE_MOUSE, 0,
      ...
      TouchX =
          Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH - 1), TouchX + mouseX);
      TouchY =
          Saturate(0, (GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT - 1), TouchY + mouseY);
      FramesWithPointer =
          (mouseX || mouseY) ? FramesWithPointerBase : FramesWithPointer;
    }
    // TOUCH: Pointer
    else if (input_cb(0, RETRO_DEVICE_POINTER, 0,
      ...
      if (hybrid_layout_scale != 1 &&
          current_layout == LAYOUT_HYBRID_BOTTOM_ONLY &&
          !hybrid_cursor_always_smallscreen) {
      ...
      if ((x >= layout.touch_x) && (x < layout.touch_x + touch_area_width) &&
        ...
        TouchX = (x - layout.touch_x) * GPU_LR_FRAMEBUFFER_NATIVE_WIDTH /
                 touch_area_width;
        TouchY = (y - layout.touch_y) * GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT /
                 touch_area_height;
      }
    }
  }
  ...
  NDS_setPad((ret & (1 << RETRO_DEVICE_ID_JOYPAD_RIGHT)),
  ...
  if ((ret & (1 << RETRO_DEVICE_ID_JOYPAD_R3)) && delay_timer == 0) {
    ...
    case LAYOUT_HYBRID_TOP_ONLY: {
      ...
      layout.dst2 = swap;
      // Need to reset Touch position to 0 with these conditions or it causes
      // problems with mouse
      if (hybrid_layout_scale == 1 && (!hybrid_layout_showbothscreens ||
                                       !hybrid_cursor_always_smallscreen)) {
        TouchX = 0;
        TouchY = 0;
      }
    ...
    case LAYOUT_HYBRID_BOTTOM_ONLY: {
      ...
      layout.dst = layout.dst2;
      layout.dst2 = swap;
      // Need to reset Touch position to 0 with these conditions are it causes
      // problems with mouse
      if (hybrid_layout_scale == 1 && (!hybrid_layout_showbothscreens ||
                                       !hybrid_cursor_always_smallscreen)) {
        TouchX = 0;
    ...
    if (colorMode == RETRO_PIXEL_FORMAT_XRGB8888) {
      if (current_layout == LAYOUT_HYBRID_TOP_ONLY ||
        ...
        if (current_layout == LAYOUT_HYBRID_TOP_ONLY) {
          ...
          SwapScreenSmall_32((u32 *)layout.dst2, screen, layout.pitch, true,
                             hybrid_layout_showbothscreens);
        } else
          SwapScreenSmall_32((u32 *)layout.dst, screen, layout.pitch, true,
                             true);

        /* Using the bottom screen as source */
        screen = (u32 *)(GPU->GetDisplayInfo().masterCustomBuffer) +
                 (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH *
                  GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT);
        if (current_layout == LAYOUT_HYBRID_BOTTOM_ONLY) {
          if (hybrid_layout_scale == hybrid_layout_ratio)
          ...
          SwapScreenSmall_32((u32 *)layout.dst, screen, layout.pitch, false,
                             hybrid_layout_showbothscreens);

          // Keep the Touch Cursor on the Small Screen, even if the bottom is
          // the primary screen? Make this configurable by user? (Needs work to
          // get working with hybrid_layout_scale==3 and
          // layout_hybrid_bottom_only)
          ...
          SwapScreenSmall_32((u32 *)layout.dst2, screen, layout.pitch, false,
                             true);
      ...
      // This is for every layout except Hybrid - same as before
        ...
        if (layout.draw_screen2) {
          screen = (u32 *)((u8 *)(GPU->GetDisplayInfo().masterCustomBuffer) +
                           GPU_LR_FRAMEBUFFER_NATIVE_WIDTH *
                               GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT * bpp);
      ...
      if (current_layout == LAYOUT_HYBRID_TOP_ONLY ||
        ...
        screen = (u16 *)(GPU->GetDisplayInfo().masterCustomBuffer) +
                 (GPU_LR_FRAMEBUFFER_NATIVE_WIDTH *
                  GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT);
        if (current_layout == LAYOUT_HYBRID_BOTTOM_ONLY) {
          if (hybrid_layout_scale == hybrid_layout_ratio)
      ...
      // This is for every layout except Hybrid - same as before
        ...
        if (layout.draw_screen1)
          SwapScreen(layout.dst, screen, layout.pitch);
        if (layout.draw_screen2) {
          screen = (u16 *)((u8 *)(GPU->GetDisplayInfo().masterCustomBuffer) +
                           GPU_LR_FRAMEBUFFER_NATIVE_WIDTH *
                               GPU_LR_FRAMEBUFFER_NATIVE_HEIGHT * bpp);
          SwapScreen(layout.dst2, screen, layout.pitch);
          DrawPointer(layout.dst2, layout.pitch);
  ...
  if (opengl_mode) {
#ifdef HAVE_OPENGL
    if (!skipped) {
      GLint drawfb = 0, readfb = 0, drawb = 0, readb = 0, active_texture = 0,
            program = 0;
      ...
      glBufferData(GL_PIXEL_UNPACK_BUFFER, layout.width * layout.height * bpp,
                   NULL, GL_STREAM_DRAW);
      void *pbo_buffer;
      if (glMapBufferRange)
        pbo_buffer = glMapBufferRange(
            GL_PIXEL_UNPACK_BUFFER, 0, layout.width * layout.height * bpp,
      ...
      if (current_texture_width != layout.width ||
          current_texture_height != layout.height) {
        glTexImage2D(GL_TEXTURE_2D, 0, internal_format, layout.width,
                     layout.height, 0, texture_type, texture_format, 0);
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, layout.width, layout.height,
                        texture_type, texture_format, 0);
      }
      ...
      glBlitFramebuffer(0, 0, layout.width, layout.height, 0, 0, layout.width,
                        layout.height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
    ...
    video_cb(skipped ? 0 : RETRO_HW_FRAME_BUFFER_VALID, layout.width,
             layout.height, 0);
#endif
  } else
    video_cb(skipped ? 0 : screen_buf, layout.width, layout.height,
             layout.pitch * 2);
  ...
  if (opengl_mode) {
    if (!environ_cb(RETRO_ENVIRONMENT_SET_HW_SHARED_CONTEXT, NULL)) {
      log_cb(RETRO_LOG_WARN,
             "Couldn't set shared context. Some things may break.\n");
    }
    ...
    if (!found_gl_context) {
      log_cb(RETRO_LOG_ERROR,
             "Couldn't create rendering context. Using software rasterizer.\n");
      opengl_mode = false;
      bpp = 2;
      colorMode = RETRO_PIXEL_FORMAT_RGB565;
...
size_t retro_get_memory_size(unsigned type) {
  if (type == RETRO_MEMORY_SYSTEM_RAM)
    return CommonSettings.ConsoleType == NDS_CONSOLE_TYPE_DSI ? 0x1000000
                                                              : 0x0400000;
  else
    return 0;
}
